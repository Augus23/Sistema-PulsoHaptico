...
Modo de prueba perceptual
-------------------------
Para forzar una política independientemente del BPM recibido:

    python pc_haptic_app.py --port COM5 --catalog patterns --manual-policy breath

Para reenviar esa misma política cada 15 segundos durante una prueba:

    python pc_haptic_app.py --port COM5 --catalog patterns --manual-policy breath \
        --manual-repeat --resend-after 15
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--manual-policy", choices=sorted(VALID_POLICIES),
                        help="Fuerza una política para test perceptual, ignorando BPM y baseline. Ej.: breath")
    parser.add_argument("--manual-repeat", action="store_true",
                        help="En modo --manual-policy, permite reenviar la política usando --resend-after.")
...
def build_and_send_policy(
    ...
    """Construye y envía una política concreta, sin depender de la telemetría."""
    if policy not in catalog:
        print(f"[MANUAL] no hay patrón JSON para {policy}; no se envía nada.")
        return False
...
def main() -> int:
    ...
    if args.manual_policy:
        print(f"[MANUAL] política forzada para test perceptual: {args.manual_policy}")
        if args.manual_repeat and args.resend_after <= 0:
            print("[MANUAL] ATENCIÓN: --manual-repeat requiere --resend-after mayor que 0; se enviará sólo una vez.")
        if build_and_send_policy(
            ser,
            catalog,
            args.manual_policy,
            intensity_overrides,
            ...
            last_sent_policy = args.manual_policy
    ...
    try:
        while True:
            ...
            if args.manual_policy:
                # En modo manual, la telemetría se muestra pero no decide la política.
                # Si no se pidió repetición, el patrón ya fue enviado al iniciar.
                if not args.manual_repeat:
                    continue
                policy = args.manual_policy
            else:
            ...
            if should_send:
                if args.manual_policy:
                    print(f"[MANUAL] reenvío de política forzada: {policy}")
                if build_and_send_policy(
                    ser,
                    catalog,
                    policy,
